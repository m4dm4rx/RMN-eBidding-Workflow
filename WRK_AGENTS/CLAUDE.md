...
- Finance Capture — RUNBOOK.md/service.yaml/store.js อ่านครบแล้ว (เดิมค้าง, ตอนนี้เสร็จ):
  ...
  - **09-13 update (Raven จาก Lord COMMANDER)**: ยืนยัน source divergence จริง — `origin/main@9563b69` ≠ deployed LIFF working copy · โค้ดมีอยู่จริงแต่ deploy แยกจาก git ที่รู้จัก path/repo ที่แท้จริงยังไม่ทราบ (ตอบบางส่วนข้อ 4 ยังไม่ปิด)
  ...
  - **09-13 update**: capability list ยืนยันจาก Raven — `can_submit`/`can_view_transactions`/`can_view_balance`/`can_edit`/`can_delete` (บริบทเพิ่มข้อ 3 แต่ mapping รายคนยังรอ Marx/แม่ ยังไม่ปิด) · ข้อ 1,2 ไม่มีข้อมูลใหม่
