...
- 🔴 **บัญชีที่ Claude connector ต่ออยู่ = `dorpnightmare@gmail.com` → ค้น `from:process@gprocurement.go.th` (รวม spam+trash) = 0 ฉบับ · ค้น `แจ้งผลการจัดซื้อจัดจ้าง`/`ตรวจรับงาน` = 0 ฉบับ** · เมลจริงอยู่ในอีกบัญชีที่ยังไม่ได้ต่อ → **checker ตัวเก่าไม่เคยพัง มันแค่ไปเฝ้ากล่องที่ไม่มีเมล**
- ⏸️ **ทางแก้ ต้องเลือกก่อนสร้าง watcher ใหม่** ① ต่อ Gmail connector เข้าบัญชีที่รับเมล e-GP จริง หรือ ② ตั้ง auto-forward ใน Gmail ของบัญชีนั้น → ส่งต่อเข้าบัญชีที่ต่ออยู่ (ไม่ต้องแตะ config Claude) · **แนะนำ ②** ทำครั้งเดียวจบ ไม่กระทบ connector อื่น · ⛔ ห้ามสร้าง watcher จนกว่าจะเห็นเมล `process@` โผล่ในบัญชีที่ต่ออยู่จริงอย่างน้อย 1 ฉบับ
